...
@Table(name = "project_operation_log", schema = "zhiyanactivelog",
        indexes = {
                ...
                @Index(columnList = "project_id, operation_time")
        }
)
@Getter
@Setter
...
public class ProjectOperationLog {
     ...
     * 项目名称（冗余字段，便于查询）
     */
    @Column(name = "project_name", length = 200, columnDefinition = "VARCHAR(200)")
     ...
     * 操作模块
     */
    @Column(name = "operation_module", nullable = false, length = 50, columnDefinition = "VARCHAR(50) NOT NULL DEFAULT '项目管理'")
